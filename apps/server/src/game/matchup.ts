...
// ── Moments (stub for step 2; fleshed out in step 4) ─────────────────────────
...
async function computeMoments(
  ...
  // Step 4 will add bad_beat / cooler detection. For step 2 we return only the
  // biggest-pot moment so the section isn't empty.
  const row = await db.execute<{
    ...
    SELECT
      h.hand_id, h.pot, m.blind_big, h.winner_user_id,
    ...
    ORDER BY (h.pot::float / m.blind_big) DESC
    LIMIT 1
  `);
  ...
  const myHole = isUserA ? JSON.parse(r.user_a_hole) : JSON.parse(r.user_b_hole);
  const oppHole = isUserA ? JSON.parse(r.user_b_hole) : JSON.parse(r.user_a_hole);
  ...
  const winnerName = winnerIsYou ? 'You' : firstName(opponentDisplayName);
  const copy = `Biggest pot — ${winnerName} took ${potBb} BB`;
